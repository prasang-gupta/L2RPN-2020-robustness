...
from .Back_Backup_agent import Back_Backup_agent
...
class Backup_agent(BaseAgent):
    ...
    def __init__(self, action_space):
        ...
        self.backup_agent2 = Back_Backup_agent(action_space)
    ...
    def get_action(self, state):
        ...
        if done_ or sum(((( obs_.rho - 1) )[obs_.rho > 1.01]))>0:
            reward_simu_2, policy_chosen_backup = self.backup_agent2.get_action(state)
            
            if  np.max(reward_simu_2)>reward_simu_:
                #print(np.max(reward_simu1))
                return reward_simu_2, policy_chosen_backup # origin     
